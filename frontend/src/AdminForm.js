  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingData && editingData.id) {
        await axios.put(`${API_BASE_URL}/api/data/${editingData.id}/`, newData, config);
        alert('Data updated!');
      } else {
        await axios.post(`${API_BASE_URL}/api/data/`, newData, config);
        alert('Data added!');
      }
  ...
  return (
    <div className="admin-form-container" data-testid="admin-form">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {editingData && <button type="button" className="cancel-btn" onClick={clearEdit}>Cancel</button>}
      </form>
    </div>
  );
};
